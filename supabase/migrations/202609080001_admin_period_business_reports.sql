...
create function public.management_period_daily_income(
  selected_from date,
  selected_to date,
...
begin
  ...
  if selected_from is null
     or selected_to is null
     or selected_from > selected_to
     or selected_to > current_lima_date
     or selected_to - selected_from > 30 then
    raise exception using errcode = '22023', message = 'REPORT_PERIOD_INVALID_RANGE';
  end if;
  ...
  with calendar as (
    ...
    from pg_catalog.generate_series(
      selected_from::timestamp,
      selected_to::timestamp,
      interval '1 day'
...
create function public.management_period_location_income(
  selected_from date,
  selected_to date,
  selected_location_id uuid default null
)
...
begin
  ...
  left join public.work_shifts
   ...
   and work_shifts.operational_date between selected_from and selected_to
  left join payment_totals on payment_totals.work_shift_id = work_shifts.id
  where (
    selected_location_id is null
...
create function public.management_period_product_sales(
  selected_from date,
  selected_to date,
  selected_location_id uuid default null
)
...
revoke all on function public.management_period_daily_income(date, date, uuid)
  from public, anon, authenticated;
revoke all on function public.management_period_location_income(date, date, uuid)
  from public, anon, authenticated;
revoke all on function public.management_period_product_sales(date, date, uuid)
  from public, anon, authenticated;

grant execute on function public.management_period_daily_income(date, date, uuid)
  to authenticated;
grant execute on function public.management_period_location_income(date, date, uuid)
  to authenticated;
grant execute on function public.management_period_product_sales(date, date, uuid)
  to authenticated;

comment on function public.management_period_daily_income(date, date, uuid) is
  'Returns admin-only daily payment income and shift coverage for a validated date interval of at most 31 days.';
comment on function public.management_period_location_income(date, date, uuid) is
  'Returns admin-only closing income grouped by location for a validated date interval.';
comment on function public.management_period_product_sales(date, date, uuid) is
  'Returns admin-only calculated product quantities for closed shifts in a validated date interval.';
