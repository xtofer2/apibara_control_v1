  ...
  return (
    <div className="space-y-8">
      ...
      <CloseShiftForm
        ...
        currentLimaDate={getLimaShiftDate()}
              ...
              return (
                <article className="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm" key={closing.id}>
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <p className="flex items-center gap-2 font-semibold text-stone-950"><MapPin aria-hidden="true" className="size-4" />{closing.shift?.location?.name ?? "Sede registrada"}</p>
                      <p className="mt-2 flex items-center gap-2 text-sm text-stone-700"><Clock3 aria-hidden="true" className="size-4" />Fecha operativa: {closing.shift?.operational_date ? formatShiftDate(closing.shift.operational_date) : "No disponible"}</p>
                      <p className="mt-1 text-xs text-stone-500">Registrado en servidor: {dateTime.format(new Date(closing.created_at))} · {closing.creator?.full_name ?? "Usuario registrado"}</p>
                    </div>
